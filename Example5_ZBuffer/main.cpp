...
bool SetupScene() { //Set up our Scene
	...
	const char* pathToVertexShader = "..\\Example5_ZBuffer\\VertexShader.h";
	ID3DXBuffer* pCode;
	D3DXCompileShaderFromFile(pathToVertexShader,NULL, NULL, 
		"MainVS", "vs_2_0", dwShaderFlags, &pCode, NULL, &g_pConstantTable);

	// Create the vertex shader
	d3dDevice->CreateVertexShader((DWORD*)pCode->GetBufferPointer(), &g_pVertexShader);
	pCode->Release();

	//Use lines to create polygons
...
bool RenderScene(float timeDelta)
{
	if (d3dDevice)
	{
		// Set up the vertex shader constants for first quad
		const float pfFirstQuadColor[] = { 1.0f, 0.0f, 0.0f, 1.0f };
		g_pConstantTable->SetFloatArray(d3dDevice, "vColor", pfFirstQuadColor, 4);
		const float pfFirstQuadPositionOffset[] = { 0.0f, 0.0f, 0.0f };
		g_pConstantTable->SetFloatArray(d3dDevice, "vPosOffest", pfFirstQuadPositionOffset, 3);
		...
		// Set up the vertex shader constants for first quad
		const float pfSecondQuadColor[] = { 0.0f, 1.0f, 0.0f, 1.0f };
		g_pConstantTable->SetFloatArray(d3dDevice, "vColor", pfSecondQuadColor, 4);
		const float pfSecondQuadPositionOffset[] = { 0.01f, -0.01f, 1.0f };
		g_pConstantTable->SetFloatArray(d3dDevice, "vPosOffest", pfSecondQuadPositionOffset, 3);


		// Draw second time ( second quad )
		d3dDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 2);
